...
| Hub-to-Hub AEL link | An AEL link between two AEL routers, carrying the unchanged event frame so events can be routed across more than one Module Hub. The topology formed by these links is acyclic in this version. Carried on a Module Port connector and one AURIORA Link Cable, which carries a link in each direction. Independent of `MCL` cascading, which remains open. |
...
| MCI transport binding | A versioned specification of how MCI is carried over one transport: framing, request correlation, error signaling, flow control and, where the transport is a physical link, its electrical layer. A binding defines carriage only; it SHALL NOT change the meaning of an MCI operation. A direct local transport such as USB and the Module Control Link are bindings of the same MCI; the direct local transport is also the host-facing interface of a Module Hub. |
| Module Control Link (MCL) | The AURIORA wired differential MCI transport binding between a Module Hub's Module Port and a Module or a downstream Module Hub: point-to-point, half duplex on one differential pair with the Module Port end as polling master, one link per Module Port, carried in the Module Port; cascaded Hub to Hub with path addressing ([AES-HUB-005](./03-architecture.md#aes-hub-005-cascading-and-path-addressing)). Its electrical profile, framing and bit rate are not fixed in this version of AES. |
| Module Port | The single connector through which a Module communicates with a Module Hub or, for AEL events only, directly with one peer Module: an M8 8-position female connector carrying the Module's MCL, AEL IN and AEL OUT as electrically independent links, plus a reference and one reserved contact, with the same contact assignment on every device. Connected only by an AURIORA Link Cable. Carries no primary Module power. A Module has at most one; there is no separate AEL connector. Specified in [`docs/interfaces/module-port.md`](./interfaces/module-port.md). |
| AURIORA Link Cable | The only cable for a Module Port: M8 8-position male-to-male, crossing AEL OUT to AEL IN with polarity preserved and everything else straight, so that one non-oriented cable connects Module to Hub, Module to Module and Hub to Hub. A generic straight-through cable is not a Link Cable and is not compatible. |
| Module Power Interface | The primary external power input of a Module: an M8 3-position male connector at a nominal 12 V DC — pin 1 `MPI_VIN`, pin 3 `MPI_RTN`, pin 4 reserved — fed by a female source output over a male-to-female power cable. Every Module regulates its own rails from it. Separate from the Module Port, from USB power and from the Unit Interface's managed power. Specified in [`docs/interfaces/module-power.md`](./interfaces/module-power.md). |
| Module Hub | Optional Platform infrastructure: an active-star device presenting one Module Port per connected Module. It combines a managed MCL concentrator with an AEL router and, optionally, an AEL event source. It carries MCI and routes AEL events without interpreting Module-specific experiment semantics, and it does not reach Units. It does not power the Modules on its Module Ports and is powered through its own power input. The host reaches it, and every Module on its Module Ports, through its host-facing interface, and learns its identity, its Module Port and Hub-to-Hub link counts and its router capabilities from the Hub itself ([AES-HUB-004](./03-architecture.md#aes-hub-004-hub-identity-and-capability-discovery)). |
...
| Host-facing interface | The connection through which the system host reaches a Module Hub — the Hub's own management contract and, through its `MCL` concentrator, the MCI of every Module on its Module Ports. An MCI transport binding, distinct from Module Ports and Hub-to-Hub AEL links, carrying no AEL frames; USB is its first realization, with the Hub as the USB device and the computer as the USB host. Defined by [AES-HUB-003](./03-architecture.md#aes-hub-003-host-facing-interface-and-traffic-separation). |
| Hub-to-Hub link port | A Module Port connector on a Module Hub whose `MCL` end is a responder: the Hub's uplink. Cabled to an upstream Hub's Module Port it carries cascaded `MCL` and a Hub-to-Hub AEL link; cabled to another link port it carries AEL only. A Hub accepts MCI from one upstream at a time. Defined by [AES-HUB-005](./03-architecture.md#aes-hub-005-cascading-and-path-addressing). |
| Stored object | A content object a Module holds and lists in its inventory with identity, type, completion state and, once complete, size and a content integrity value: a recording, an event log, a diagnostic record, or a committed Asset or Session. A completed stored object is immutable. |
| Recording | A stored object holding the measurement data a Module acquired locally during exactly one run segment, with the metadata that makes it a dataset: run and segment, deployment and Session identity, local-time and sample-index range, completion state, data format identifier and version, and the firmware version that produced it. Made alone in the field or under a Module Hub, it is the same kind of object. |
| Stored object retrieval | An optional MCI function ([AES-MCI-008](./05-interfaces-and-versioning.md#aes-mci-008-stored-object-retrieval)): a host enumerates a Module's stored objects and reads them by identity, offset and length in bounded chunks. Stateless, host-paced and read-only; a retrieval is complete when the assembled object matches its integrity value. |
| Bulk transfer | The movement of an Asset, Session, firmware image or stored object over MCI in bounded chunks — throughput-oriented and latency-tolerant. One of the two classes of MCI traffic; the other is control. A bulk transfer never holds a link for longer than one chunk and never uses AEL. |
| Live measurement stream | Continuous delivery of measurement data to a host while acquisition runs. Distinguished from an AEL event, from polled telemetry and from a recording read back afterwards. No Platform mechanism exists for it in this version of AES; a Module that offers one declares it. |
