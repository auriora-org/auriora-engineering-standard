...
| Module Lifecycle State | The Module-level state a host observes and advances over MCI: `IDLE → CONFIGURED → ARMED → RUNNING → COMPLETE`. `ARMED` is the boundary between preparation over MCI and deterministic execution on AEL; which AEL receive bindings are enabled in `ARMED` and in `RUNNING` is per-binding configuration. Firmware MAY use its own state names; the states and the boundary are what the contract fixes. |
