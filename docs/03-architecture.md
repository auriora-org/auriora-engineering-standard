...
A Module Hub holds no deployment policy and no experiment state of its own. Its active route table is not reinstated after a Hub reset ([AES-AEL-005](./05-interfaces-and-versioning.md#aes-ael-005-active-hub-routing-and-bounded-overload-behavior)), so an unattended deployment whose events must cross a Hub survives the reset of a Module that recovers under [AES-MCI-007](./05-interfaces-and-versioning.md#aes-mci-007-recovery-after-reset-and-run-segment-provenance), but not a Hub power loss without a host to re-commit the routes. Modules wired directly to one another, or a single Module on its own, carry no such dependency; whether a Hub should ever hold a persistent route policy is a separate open question ([EDR-009](./edr/EDR-009-autonomous-module-operation-and-recovery.md)).

Two kinds of scaling are kept apart. **AEL scales through Hub-to-Hub AEL links**: a router output driving another router's input, carrying the unchanged frame, in an acyclic topology ([`docs/interfaces/ael.md`](./interfaces/ael.md) Section 8.6). This needs no decision about `MCL`. Whether **`MCL` may be cascaded** — a Module Port feeding another Hub's management path rather than a Module — remains open and is settled together with the `MCL` binding, because it decides whether that binding's addressing is flat or hierarchical. In this version a host reaches every Hub through that Hub's own host-facing transport, and supports several Hubs at once ([AES-HUB-002](#aes-hub-002-port-independence-and-scale-interoperability)).
...
- **It does not replace the standalone AEL ports.** A Module keeps its own AEL IN and AEL OUT connectors as specified in [`docs/interfaces/ael.md`](./interfaces/ael.md). That is what preserves direct Module-to-Module operation without infrastructure, test fixtures, and adapters to laboratory equipment.
