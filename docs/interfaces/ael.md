...
- **Semantic names stay off the wire.** Names such as `detector_a_fired` or `phase_complete` exist in the experiment definition and in Module documentation; the host maps them to identifiers before the run and stores the mapping in the deployment record so that every logged `event_id` can be decoded afterwards.
...
- Loss of router power, or a router reset, SHALL leave every output in AEL idle; on recovery the router does **not** reinstate an active table on its own — the host verifies and re-commits, exactly as a Module is never re-armed by reconnection ([AES-MCI-004](../05-interfaces-and-versioning.md#aes-mci-004-module-lifecycle-and-the-arm-boundary)).
