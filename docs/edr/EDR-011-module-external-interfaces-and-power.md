...
- **Two `MCL` masters on one pair** when two Hubs' Module Ports are cabled together, and the **`GND` return-current bound** on the Module Port with several independently powered Modules on one Hub — both recorded as open in the Module Port specification, decided with the `MCL` binding and bring-up measurement respectively.
