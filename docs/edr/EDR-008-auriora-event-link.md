...
4. **Events cross Hubs.** An installation larger than one Hub's port count needs an event on one Hub to reach a Module on another, without waiting for the `MCL` cascading question that EDR-007 and the 0.7.1 patch deliberately left open.
...
| **A `PATCH` AES version** — the SYNC specification was Draft | Rejected. The Draft was the physical layer; the normative model in Interfaces and Versioning §4 stated that SYNC carries no identifier and defined the Hub's behavior, and both change incompatibly. [AES-VER-001](../05-interfaces-and-versioning.md#aes-ver-001-semantic-versioning-for-released-contracts) defines breaking by impact, and a clarification-level number would hide it. |
| **AES `0.8.0`** | **Chosen.** AES is in pre-Release `0.x` versioning, which [AES-VER-001](../05-interfaces-and-versioning.md#aes-ver-001-semantic-versioning-for-released-contracts) leaves informal, so an incompatible change does not force a `MAJOR` increment; the incompatibility is stated in this record and in the changelog entry rather than left to the number. A `1.0.0` would claim a stability the Platform's Draft interface specifications do not have. |
...
11. `AES-SYNC-001` to `AES-SYNC-004` and the document identifier `AES-EXAMPLE-SYNC` are retired; `docs/interfaces/sync.md` becomes a superseded notice pointing to the last AES release that carried the full SYNC specification (`v0.7.1`). AES is released as **`0.8.0`**.
...
- **This is an incompatible normative change, released as AES `0.8.0`.** AES is in pre-Release `0.x` versioning, so the increment is not a `MAJOR` one; no Released Module or Hub exists, so nothing deployed breaks, and the incompatibility is recorded here and in the changelog rather than signalled by the number alone.
