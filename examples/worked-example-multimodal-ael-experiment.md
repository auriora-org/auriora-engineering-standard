...
- **AASM** — Audio Module, plays acoustic bursts; can change playback phase while running.
...
then, for 10 trials:
    when APEM detects a spike on channel A (refractory 20 s, max 10 detections):
        AASM plays burst B3 immediately
        US emits one burst
        APBM switches to red phase
    when APBM reports the red phase complete:
        AASM changes to phase 2 if still playing
        APEM marks it
    when AASM or US completes:
        APEM marks it
recovery 120 s
```
...
```text
APEM  serial 0x2A41  fw 1.3.0  AEL 0.1  AEL IN yes  AEL OUT yes  rx_bindings ≤ 8  src_bindings ≤ 4  autonomous_continuation yes
AASM   serial 0x1107  fw 2.0.1  AEL 0.1  AEL IN yes  AEL OUT yes  rx_bindings ≤ 4  src_bindings ≤ 2  autonomous_continuation yes
US    serial 0x0C12  fw 0.4.0  AEL 0.1  AEL IN yes  AEL OUT yes  rx_bindings ≤ 2  src_bindings ≤ 1  autonomous_continuation no
APBM  serial 0x3390  fw 1.1.0  AEL 0.1  AEL IN yes  AEL OUT yes  rx_bindings ≤ 4  src_bindings ≤ 2  autonomous_continuation yes
...
| `aasm.burst_complete` | AASM | `0x0301` |
...
AASM   session = B3      asset B3 verified present
...
AASM   0x0001 in ARMED → START_SESSION → RUNNING
...
For 60 s nothing travels on AEL. APEM records, AASM holds its silent context, APBM runs white light — each from its own Session against its own clock. The Hubs count zero frames.
...
H1    P1 RX 0x0201 → validated → P2 (AASM) and LINK→H2, launched within skew
H2    LINK←H1 RX 0x0201 → validated → P1 (US), P2 (APBM)
AASM   0x0201 in RUNNING → PLAY_BURST   log: RX 0x0201 ACTED  frame ref t=…  action PLAY_BURST
...
AASM   burst ends → src → 0x0301 → H1 P2 → H1 P1 → APEM: INSERT_MARKER
APBM  red phase ends → src → 0x0303 → H2 P2 → LINK→H1 → H1: P1 (APEM marker), P2 (AASM CHANGE_PHASE(2) — while RUNNING)
                                     → H2 P1 (US)
```

AASM changes phase mid-playback because that binding is enabled in `RUNNING`; a Module built to the old ignore-while-running rule could not have done this.
...
At trial 4 the host's transport to H1 drops. Nothing on AEL notices: routes are committed, Modules are armed and running, events flow between them exactly as before. APEM, AASM and APBM have declared autonomous continuation with host-loss behavior *continue* ([AES-MCI-006](../docs/05-interfaces-and-versioning.md#aes-mci-006-autonomous-continuation-and-deployment-policy)) and carry on; US continues too — it needs no host to react to `0x0201` — but it promised nothing about finishing, and had it faulted, no host was there to recover it, which is what its `autonomous_continuation no` declaration means. A worked example of a single Module deployed this way on purpose, for weeks, is [Unattended Field Deployment](./worked-example-unattended-field-deployment.md).
...
APEM's Session stops recording at its scheduled end; `COMPLETE → IDLE` (one-shot). AASM and US return to `ARMED` after each burst (repeat-armed) and are disarmed by the host at the end. The host stages nothing new on the Hubs until every dependent Module has left `ARMED`/`RUNNING`.
...
```text
run 2026-09-14/07
modules:   APEM 0x2A41 fw1.3.0 rev B  session REC-7 hash 91e…   AASM 0x1107 …   US 0x0C12 …   APBM 0x3390 …
ael:       version 0.1
event map: 0x0001 t0 · 0x0201 apem.spike_a · 0x0202 apem.acquisition_started · 0x0301 aasm.burst_complete · 0x0302 us.burst_complete · 0x0303 apbm.red_phase_complete
bindings:  (per Module, as committed — see step 3)
hubs:      H1 fw 0.9.2 routes a41c…9e · H2 fw 0.9.2 routes 7f02…b3 · link H1↔H2
timing:    H1/H2 published envelopes; APEM frame-to-marker 63 µs ±0.3 (60 µs frame + gap floor, then capture-to-marker); AASM frame-to-burst 104 µs ±2 (floor + pipeline, compensated)
counters:  per device, per port, per direction; 1 invalid frame at H1 LINK←H2 (trial 6); US ignored 11 (1 × 0x0001, 10 × 0x0303)
logs:      APEM acquisition with markers by sample index; AASM/US/APBM event logs; Hub counters
```
...
```text
APEM  AEL OUT ──────────────────► AEL IN  AASM

APEM  src DETECTOR_A_FIRED → 0x0201                 AASM  rx 0x0201 → PLAY_BURST  enabled in ARMED, RUNNING
                                                     rx 0x0001 → (not used; AASM is started over MCI here)
```

The host starts APEM's acquisition and AASM's context over MCI (inter-Module timing of the *start* does not matter here), and from then on every detected spike is a `0x0201` frame that AASM acts on within its documented frame-to-action latency. Same firmware, same bindings, same frame, no Hub, no route table — and an identifier that is unique in a routing domain of two.
